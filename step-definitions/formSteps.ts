...
When("user fills the form with following data:",async function (this: CustomWorld, dataTable: DataTable) 
{
...
When("user fills the form with name {string} and email {string}",async function (this: CustomWorld, name: string, email: string) 
{
...
Then("form should be submitted successfully",async function (this: CustomWorld) 
{
    await this.formPage.assertFormSubmittedSuccessfully();
});


Then("form submission result should be {string}",async function (this: CustomWorld, expectedResult: string) 
{
